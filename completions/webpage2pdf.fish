...
for cmd in webpage2pdf w2p
    ...
    complete -c $cmd -s j -l jobs -x      -d "Fetch this many pages at once"
    complete -c $cmd -l no-cache         -d "Do not use the fetch cache"
    complete -c $cmd -l refresh          -d "Re-fetch despite a cached copy"
    complete -c $cmd -l cache-info       -d "Show what the cache holds"
    complete -c $cmd -l clear-cache      -d "Empty the cache"
    complete -c $cmd -l fix              -d "With --doctor, attempt repairs"
